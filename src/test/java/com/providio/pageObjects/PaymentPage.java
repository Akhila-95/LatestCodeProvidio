...
WebDriver lDriver;
	...
	// edit the customer
		...
		public void editShippingMethod(WebDriver driver) throws InterruptedException {
			...
			//add new address
			WebElement addNewAddress= driver.findElement(By.xpath("(//button[@class='text-center btn-add-new btn mx-2'])[1]"));
			addNewAddress.click();
			
			checkOutPage cop = new checkOutPage(driver);
			
			cop.setFisrtName(fname);
			logger.info("Entered first name");
			
			cop.setLastname(lname);
			logger.info("Entered last name");
			
			cop.setAddress1(address,driver);
			logger.info("Enter new address");
						
			cop.setPhone(phonenumber);
			logger.info("Entered phone number");
			
			cop.clickpaymentbutton(driver);
			logger.info("clciked on next payment button");
...
//paypalchekout
			...
			@FindBy(xpath ="//select[@name='braintreePaypalAccountList']")
	        WebElement newAccount;
			
			public void selectNewAccountFromDropdown() throws InterruptedException {
	        Select newcardselect = new Select(newcard);
	        newcardselect.selectByIndex(0);
	        Thread.sleep(3000);
