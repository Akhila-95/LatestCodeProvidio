...
public class tc__PdpPagewithGuestuser extends baseClass {
	...
	private void performActionsOnProductDescriptionPage() throws InterruptedException {
	 	// size selection
  		 ...
  		 //if product is bundle then no need of carousel
	...
	public void writeReviewForProduct() throws InterruptedException {
		...
		if(powerReviews.size()>0 ||yoptoReviewList.size()>0) {	
			if((powerReviews.size()>0)) {
				test.info("Power reviews are activated");
				productDescriptionPage pdp = new productDescriptionPage(driver);
				...
				test.info("Yopto review are activated");
				productDescriptionPage pdp = new productDescriptionPage(driver);
				pdp.yoptpoReviews();
			}
	    
	  }else {
		  System.out.println("No reviews are activated");
	  }
	}
